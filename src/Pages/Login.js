...
} from "../store/services/captchaService";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if ("data" in response && response.data.token) {
        setIsSuccess(true);
        const cookiesData = `${myCipher(response.data?.user?.name)}-${myCipher(
          response.data?.user?.role
        )}-${myCipher(response.data?.user?.mobile)}`;
        const skytrack_cookiesData = `${myCipher(
          response.data?.user?.email
        )}-${myCipher(response.data?.user?.role)}-${myCipher(
          response.data?.user?.date_joined
        )}-${myCipher(response.data?.user?.mobile)}`;
        sessionStorage.setItem(
          "cookiesData",
          cookiesData + "-" + response.data?.user?.id
        );
          ...
          <div className="w-full max-w-md backdrop-blur-md bg-white/25 rounded-2xl p-8 shadow-2xl border border-white/20">
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="mb-4">
                <input
                  ...
                  className={`w-full p-3 rounded-lg border ${
                    errors.phoneNumber ? "border-red-400" : "border-orange-300"
                  } bg-white/70 text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-orange-400`}
                />
                {errors.phoneNumber && (
              ...
              <div className="mb-4">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <button
                    ...
                    className="px-3 py-1 bg-orange-200 rounded hover:bg-orange-300 text-orange-700"
                  >
                    Refresh
                  </button>
                </div>
                <input
                  ...
                  className={`w-full p-3 rounded-lg border ${
                    errors.captchaReply ? "border-red-400" : "border-orange-300"
                  } bg-white/70 text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-orange-400`}
                />
                {errors.captchaReply && (
                  <p className="text-red-500 text-xs mt-1">
              ...
              <div className="mb-4 flex items-center text-gray-800 text-sm">
                ...
                />
                I accept the{" "}
                <a href="/termscondition" className="underline text-orange-600">
                  Terms
                </a>{" "}
                and{" "}
                <a href="/termscondition" className="underline text-orange-600">
